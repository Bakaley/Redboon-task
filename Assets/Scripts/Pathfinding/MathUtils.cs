...
namespace Pathfinding
{
    ...
    {
        public static bool IsNearlyEqual(float a, float b, float difference = 0.0001f)
        {
            return Mathf.Abs(a - b) <= difference;
        }

        public static bool IsIntervalContains(float value, float left, float right)
        {
            ...
            out Vector2 intersection)
        {
            ...
            if (IsNearlyEqual(cross_rs, 0f))
            {
                // Parallel lines
                if (IsNearlyEqual(CrossProduct2D(qminusp, r), 0f))
                {
